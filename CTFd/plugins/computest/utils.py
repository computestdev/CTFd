...
def get_standings(count=None, admin=False, category=None):
    """
    ...
    Two users can have the same score but one user will have a solve ID that is
    before the others. That user will be considered the tie-winner.

    Challenges & Awards with a value of zero are filtered out of the
    calculations to avoid incorrect tie breaks.

    This function was modified from :meth:`CTFd.utils.scores`. The `category`
    argument was added. The `AccountScoreVisibility` model is used to implement
    opt-in visibility on the scoreboard.
    """
    Model = get_model()
    ...
    """
    Combine awards and solves with a union. They should have the same amount of
    columns
    """
    ...
    Different databases treat time precision differently so resolve by the row
    ID instead.
    """
    if admin:
    ...
    """
    Filter accounts by visibility preference.
    """
    standings_query = standings_query.join(
        AccountScoreVisibility, Model.id == AccountScoreVisibility.account_id)
    standings_query = standings_query.filter(AccountScoreVisibility.visible == True)
...
def get_standings_per_category(count=None, admin=False):
    ...
    for (category,) in categories:
        scores[category] = get_standings(count, admin, category)
